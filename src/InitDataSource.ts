...
// alert only
const entityPath = process.env.ENV === 'production' ? path.join(__dirname + '/../../build/src/entity/**/*.entity.js') :  path.join(__dirname + '/../src/entity/**/*.entity.ts')
logger.info(`Env is: --> ${process.env.NODE_ENV}`)
logger.info(`Server Path--> ${__dirname}`)
logger.info(`Entity Path: --> ${entityPath}`)
logger.info(`DB Info: --> mysql://${process.env.MYSQL_USERNAME}:*****@${process.env.MYSQL_DB_HOST}:${process.env.MYSQL_DB_PORT}/${process.env.MYSQL_DB_NAME}`)
