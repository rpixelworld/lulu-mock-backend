	...
	static async getTaxRate(req: Request, res: Response): Promise<void> {
		...
		try {
			...
			if (taxRate) {
				...
				res.status(400).send(
					ResponseHelper.generateFailureResult(ErrorCode.TAX_RATE_NOT_FOUND, 'Tax rate not found')
				);
			}
		} catch (e) {
			logger.error('Server error:', e);
			res.status(500).send(ResponseHelper.generateFailureResult(ErrorCode.DB_ERROR, e.driverError));
		}
	}
}
