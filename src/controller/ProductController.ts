...
class ProductController {
	...
	static async uploadSearch(req: Request, resp: Response) {
		...
		if(!req.file){
			return resp.status(500).send(ResponseHelper.generateFailureResult(ErrorCode.FILE_UPLOAD_ERROR, "file upload error"));
		}
		...
		if(ext.toLowerCase()!='jpg' && ext.toLowerCase()!='png') {
			return resp.status(400).send(ResponseHelper.generateFailureResult(ErrorCode.EXTENTION_NOT_SUPPORTED, "Only jpg and png files are supported"))
		}
