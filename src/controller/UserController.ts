...
class UserController {
    ...
    static async all(req: Request, resp: Response) {
        ...
        try{
            ...
            CLog.bad("find users failed", e)
            resp.status(500).send(ResponseHelper.generateFailureResult(e.driverError))
        }
    }
    ...
    static async add(req: Request, resp: Response) {
        ...
        if(errors.length > 0){
            CLog.bad("Validation error", errors)
            return resp.status(400).send(ResponseHelper.generateFailureResult(errors))
        }
        ...
        try{
            ...
            CLog.bad("create a user failed", e)
            resp.status(500).send(ResponseHelper.generateFailureResult(e.driverError))
        }
    }
    ...
    static async one(req: Request, resp: Response) {
        ...
        // CLog.info('find user with id=', userId, Number.isInteger(userId))
        if(!Number.isInteger(Number(userId))) {
            return resp.status(400).send(ResponseHelper.generateFailureResult('Invalid user id'))
        }
        ...
        try{
            ...
            if(!user) {
                return resp.status(400).send(ResponseHelper.generateFailureResult(`User with id=${userId} not found`))
            }

            return resp.status(200).send(ResponseHelper.generateSuccessResult(user))
        }
        catch (e) {
            CLog.bad("find a user failed", e)
            resp.status(500).send(ResponseHelper.generateFailureResult(e.driverError))
        }
    }
    ...
    static async update(req: Request, resp: Response) {
        const {userId} = req.params
        CLog.info(`updating user with id=${userId}`)
        ...
        try{
            ...
            CLog.bad("update a user failed", e)
            resp.status(500).send(ResponseHelper.generateFailureResult(e.driverError))
        }
    }
    ...
    static async delete(req: Request, resp: Response) {
        const {userId} = req.params
        CLog.info(`deleting user with id=${userId}`)
        ...
        try{
            ...
            CLog.bad("delete a user failed", e)
            resp.status(500).send(ResponseHelper.generateFailureResult(e.driverError))
        }
    }
}
