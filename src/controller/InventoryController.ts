...
class InventoryController {
  ...
  static async getProductInventory(req: Request, resp: Response) {
    ...
    try {
      ...
      if (!products || products.length===0) {
        ...
        return resp.status(400).send(ResponseHelper.generateFailureResult(ErrorCode.PRODUCT_NOT_FOUND, 'product is not found'))
  ...
  static async getInventory(req: Request, resp: Response) {
    ...
    try {
      const product =await InventoryController.repo.find({where: {
        productId: productId,
        colorId: colorId,
        size: size,}});
      // verify params
  ...
  static async updateInventory(req: Request, resp: Response) {
    ...
    if (!productId || !colorId || !size || stock===undefined) {
      ...
      return resp.status(400).send(ResponseHelper.generateFailureResult(ErrorCode.INVALID_REQUEST, 'Invalid request parameters'));
    }
    // verify type of params
    if (typeof stock !== 'number') {
      logger.error('Stock is not a number');
      return resp.status(400).send(ResponseHelper.generateFailureResult(ErrorCode.VALIDATION_ERROR, 'Stock must be a number'));
    }

    try {
      // begin looking and comparing
      const product =await InventoryController.repo.findOne({where: {
          productId: productId,
          colorId: colorId,
          size: size,}});
      if (!product) {
      ...
      if (err.length>0) {
        ...
        return resp.status(400).send(ResponseHelper.generateFailureResult(ErrorCode.VALIDATION_ERROR, 'stock is not number'))
      }
      // save to the entity
