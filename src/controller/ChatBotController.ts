...
messagePayload.push({ role: "system", content: "My name is LULUBot, I am your shopping assistent. How can I help you today?" })
...
class ChatBotController {
	...
	static async userPrompt(req: Request, res: Response): Promise<any> {
		...
		if(aiResponse.choices[0].finish_reason == 'tool_calls'){
			...
			aiResults = await execustToolCall(toolCall.id, toolCall.function.name, JSON.parse(toolCall.function.arguments))
		}
		else {
			aiResults = [{
...
async function execustToolCall(toolcallId: string, toolName: string, args: any) {
	switch (toolName) {
		case 'get_recent_order':
			...
			if(!user) {
				return [{role: 'tool', content: "I can't find the email in our system, please send me the correct one. ", tool_call_id: toolcallId}]
			}
			const recentOrder: Order = await getRecentOrder(email)
			if(!recentOrder) {
				return [{role: 'tool', content: "It seems you there is no order associated with your email.", tool_call_id: toolcallId}]
			}
			else {
				const resultArr = [{role: 'tool', content: `You recent order number is ${String(recentOrder.id).padStart(10, '0')}, the status is ${AppHelper.getEnumKeyByValue(OrderStatus, recentOrder.status)}`, tool_call_id: toolcallId}]
			...
			case 'get_payment_link':
				...
				return [{role: 'tool', content: `Your payment link is <a href="${paymentLink}">Pay your order</a>`, tool_call_id: toolcallId}]

			case 'get_shippment_data':
				const order: Order = await getSingleOrder(args.orderId)
				if(!order) {
					return [{role: 'tool', content: "I can't find the order by the order number you provided, please check and reply with the correct order number. ", tool_call_id: toolcallId}]
				}
				if(order.status == OrderStatus.UNPAID) {
					return [{role: 'tool', content: "I'm sorry we are not able to ship your order, since it's not paid yet. ", tool_call_id: toolcallId}]
				}
				const plannedDate = AppHelper.formatDate(order.plannedShipmentDate)
				return [{role: 'tool', content: `We planned to ship your order at ${plannedDate}`, tool_call_id: toolcallId}]
		default:
