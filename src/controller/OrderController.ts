...
class OrderController {
  ...
  static async getUserOrders(req: Request, resp: Response) {
    ...
    if (validationMsg) {
      return resp.status(400).send(ResponseHelper.generateFailureResult(ErrorCode.VALIDATION_ERROR, validationMsg));
    }
    ...
    try {
      ...
      return resp.status(200).send(ResponseHelper.generateSuccessResult({
        pagination: ResponseHelper.generatePaginationParams(Number(pageNo), Number(pageSize), total, orders.length),
        orders: orders
      }));
    } catch (e) {
      logger.error("Exception find orders", e);
      return resp.status(500).send(
          ResponseHelper.generateFailureResult(ErrorCode.DB_ERROR, e.driverError),
      );
    }
  }
    ...
    static async getOneOrder(req: Request, resp: Response) {
        ...
        if (!Number.isInteger(Number(orderId))) {
            return resp.status(400).send(ResponseHelper.generateFailureResult(ErrorCode.VALIDATION_ERROR, 'Invalid order id'));
        }
...
function validateQueryOrder(req: Request): string {
    ...
    if (orderStatus
        && (!Number.isInteger(Number(orderStatus)) || !(Object.values(OrderStatus).includes(orderStatus)))) {
